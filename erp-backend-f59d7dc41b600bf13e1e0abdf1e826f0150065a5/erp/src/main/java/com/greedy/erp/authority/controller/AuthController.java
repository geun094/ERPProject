package com.greedy.erp.authority.controller;
...
import com.greedy.erp.authority.service.AuthService;
...
public class AuthController {
	...
	@PostMapping("/login")
	public ResponseEntity<ResponseDTO> login(@RequestBody EmpDTO empDTO) {
		log.info("제발 나와라요~ 나와라예~");
		System.out.println("오긴 온다???");
		return ResponseEntity
				.ok()
				.body(new ResponseDTO(HttpStatus.OK, "로그인 성공", authService.login(empDTO)));
...
//	@PostMapping("/signup")
//	public ResponseEntity<ResponseDTO> signup(@RequestBody EmpDTO empDTO) {	// 회원 가입 정보를 받아 냄
//		return ResponseEntity
//				.ok()
//				.body(new ResponseDTO(HttpStatus.CREATED, "회원가입 성공", authService.signup(empDTO)));
//	}
